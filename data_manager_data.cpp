...
QMap<int32_t, QMap<QString, QList<GenericData>>> DataManagerData::getGenericMap() const
{
    return m_genericMap;
}

void DataManagerData::setGenericMap(
    const QMap<int32_t, QMap<QString, QList<GenericData>>>& genericMap)
{
    m_genericMap = genericMap;
}

void DataManagerData::insertEventByID(int32_t id, const GenericData& event)
{
    // 直接通过引用修改值,不存在的key会自动添加默认的键值对
    m_genericMap[id][event.m_eventType].append(event);
}

void DataManagerData::setRefYear(int refYear)
{
    m_refYear = refYear;
}

QVector3D DataManagerData::getRefLLAPoint() const
{
    return m_refLLAPoint;
}

void DataManagerData::setRefLLAPoint(const QVector3D& refLLAPoint)
{
    m_refLLAPoint = refLLAPoint;
}
...
void DataManagerData::setMinMaxTime(double minTime, double maxTime)
{
    m_minTime = minTime;
    m_maxTime = maxTime;
}
...
void DataManagerData::insertGOGFile(const QString& fileName)
{
    m_gogFileList.append(fileName);
}

void DataManagerData::insertAttrUnit(int32_t id, const QList<QPair<QString, QString>>& attrUnitList)
{
    m_attrUnitHash.insert(id, attrUnitList);
}

void DataManagerData::insertEntityData(int32_t id,
                                       const QHash<QString, QVector<double>>& attrDataMap)
{
    m_newEntityDataMap.insert(id, attrDataMap);
}

void DataManagerData::insertEntity(int32_t id, const DataManagerData::Platform& plat)
{
    m_platformMap.insert(id, plat);
}

void DataManagerData::insertTimeData(double time)
{
    m_timeDataSet.insert(time);
}
