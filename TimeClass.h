...
{
public:
	Seconds() : m_seconds(0), m_fraction(0) {}
	Seconds(double time) { convert(time); }
	Seconds(int64_t sec, int frac) : m_seconds(sec), m_fraction(frac) { fix(); }
	Seconds(int64_t sec, double frac) { convert(frac);  m_seconds += sec; }
	Seconds(const Seconds& time) : m_seconds(time.m_seconds), m_fraction(time.m_fraction) {}
	~Seconds();

	int64_t getSeconds() const { return m_seconds; }
	double getFraction() const { return (m_fraction * OUTPUT_CONV_FACTOR); }
	int getFractionLong() const { return m_fraction; }

	operator double() const { return convert(); }
	double Double() const { return convert(); }

	Seconds& operator =(const Seconds& time);
	Seconds& operator =(double time) { convert(time); return *this; }
	Seconds& operator ++() { m_seconds += 1; return *this; }
	Seconds& operator --() { m_seconds -= 1; return *this; }
...
public:
	...
	int referenceYear() const { return m_referenceYear; }
	Seconds secondsSinceRefYear() const { return m_secondsSinceRefYear; }
	...
	TimeStamp& operator ++() { m_secondsSinceRefYear += Seconds(1, 0); fix_(); return *this; }
	TimeStamp& operator --() { m_secondsSinceRefYear -= Seconds(1, 0); fix_(); return *this; }
	...
	*/
	...
	void getTimeComponents(unsigned int& day, unsigned int& hour, unsigned int& min, float& fsec) const;
private:
	int m_referenceYear;
...
{
public:
	QString toString(double secSinceRefYear, int refYear);
	bool isValidIntNumber(QString str);
	bool canConvert(const QString &timeString, QStringList &listTime);
	void convertToTime(QString timeString, int &days, int &refYear, int &hour, int &minute, double &seconds);
	double convertToSeconds(QString timeString, int refYear);
	void secondsConvertToTime(double secSinceRefYear, int refYear, unsigned int &days, unsigned int &hour, unsigned int &minute, float &fsec);
};